...
namespace BiatecMCP.BusinessLogic
{
    ...
    public sealed class AlgorandChainRegistry : IAlgorandChainRegistry
    {
        ...
        // BiatecMCP has no Redis by design (see CLAUDE.md's "MCP server" architecture note) - this is only
        // ever liveness-check data, not security-sensitive, so a per-pod in-memory cache is the right fit
        // rather than reintroducing a distributed cache dependency just for this.
        ...
        private async Task<IReadOnlyList<AlgorandChain>> DiscoverSupportedChainsAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var entry in genesisList)
            {
                ...
                {
                    continue; // placeholder entry (e.g. a local sandbox) - not a real chain to bridge/pay on
                }
