...
The Google Drive self-custody primitives (`GoogleDriveRepository`, `DriveService`,
`AesEncryptionHelper`, `Configuration`/`AesOptions`) live in the sibling `BiatecSelfCustodyCore/`
class library, which this project references — they're shared with the separately-deployed
`BiatecOIDC` service (see the repo root `CLAUDE.md` for why). The OIDC/JWT identity provider
itself now lives entirely in `BiatecOIDC/` — see `BiatecOIDC/README.md`.
