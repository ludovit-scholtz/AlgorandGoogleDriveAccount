...
Biatec MCP Server — an ASP.NET Core 10 (`AlgorandGoogleDriveAccount`) service that gives AI assistants (via the Model
...
- When bumping the target .NET version, update it everywhere, not just the `.csproj` files: both base images in
  ...
  `mcr.microsoft.com/dotnet/sdk:<ver>`), the version mentioned in this file and
  `.github/copilot-instructions.md`, and `AlgorandGoogleDriveAccount/README.md`. A `TargetFramework` bump alone
  will build fine locally but ship a Docker image on the old runtime.
