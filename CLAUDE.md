...
- **MCP server**: `BiatecMCP` is a pure OAuth 2.1 *resource server* (RFC 9728 Protected Resource Metadata + RFC
  ...
  (`AddMcpServer(options => { options.ServerInstructions = ...; }).WithHttpTransport().WithToolsFromAssembly()
  .AddAuthorizationFilters()`), stateless HTTP transport. `ServerInstructions` (`BiatecMCP/Program.cs`) is a
