...
{
    /// <summary>
    /// Persists which (<c>primaryAddress</c>, <c>slot</c>) key material signs for a given address, so wallet
    ...
    public interface IAddressActivationService
    {
        ...
        /// <summary>
        /// Records that <paramref name="primaryAddress"/>/<paramref name="slot"/> signs for
        ...
        /// </summary>
        Task<AddressActivationEntry> ActivateAsync(string email, string provider, string? accessToken, string address, string family, string primaryAddress, int slot, CancellationToken cancellationToken = default);
