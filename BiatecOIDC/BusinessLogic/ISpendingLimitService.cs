namespace BiatecOIDC.BusinessLogic
{
    ...
    {
        /// <summary>
        /// The caller's currently configured limits for one bucket. <paramref name="seedAddress"/>
        /// <c>null</c> (the default) selects the account-wide global bucket; a non-null value selects the
        /// per-address bucket for that <c>(seedAddress, slot)</c> signing identity. Returns an all-zero
        ...
        /// </summary>
        Task<SpendingLimitSettings> GetLimitsAsync(string email, string provider, string? accessToken, string? seedAddress = null, int slot = 0, CancellationToken cancellationToken = default);

        /// <summary>
        /// Persists new limits for one bucket - same <paramref name="seedAddress"/>/<paramref name="slot"/>
        ...
        Task SetLimitsAsync(string email, string provider, string? accessToken, SpendingLimitSettings settings, string? seedAddress = null, int slot = 0, CancellationToken cancellationToken = default);

        /// <summary>
        /// Checks whether signing a group worth <paramref name="amountUsd"/> (as priced by
        /// <c>IAssetValuationService</c>) with the identity <paramref name="seedAddress"/>/<paramref name="slot"/>
        ...
        Task EnsureWithinLimitsAsync(string email, string provider, string? accessToken, decimal amountUsd, string seedAddress, int slot, CancellationToken cancellationToken = default);
